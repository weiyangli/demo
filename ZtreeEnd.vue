    ...
    methods: {
        ...
        onDrop(event, treeId, treeNodes, targetNode, moveType, isCopy) {
            ...
            if (treeNodes.length > 0) {
                ...
                // 2 获取当前父节点下，除拖拽节点外的所有子节点排序
                let nodes = this.zNodes.filter(x=> x.pId == treeNodes[0].pId);
                // 3 拖拽后查看上下节点个数更改节点顺序
                let topNum = $('#'+treeNodes[0].tId).prevAll().length;
                let bottomNum = $('#'+treeNodes[0].tId).nextAll().length;
                // 3-1 计算拖拽节点的序号
                thisNode.sort = nodes.length - bottomNum + 1;
                // 3-2 将同级下面的节点序号加一
                for (let node of nodes) {
                    if (node.sort >= thisNode.sort) {
                        node.sort += 1;
                    }
                }
                // 4 将排好序的数据放回 zNodes
